...
const ChatAutomationsAdmin: React.FC = () => {
  ...
  const loadRules = async () => {
    ...
    try {
      const data = await ChatService.getAutomations();
      setRules(data);
    } catch (error) {
  ...
  const loadJobs = async () => {
    setLoadingJobs(true);
    try {
      const data = await ChatService.getAutomationJobs();
      setJobs(data);
    } catch (error) {
  ...
  const toggleRule = async (id: string) => {
    ...
    try {
      await ChatService.saveAutomation(updated);
    } catch (error) {
  ...
  return (
    <div className="p-4 md:p-8 max-w-[1600px] mx-auto space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex bg-slate-100 p-1 rounded-2xl w-fit">
       ...
       {activeTab === 'reglas' ? (
         loadingRules ? (
           <div className="py-12 text-center text-xs font-bold text-slate-400">Cargando automatizaciones...</div>
         ) : (
               ...
               <div key={rule.id} className="bg-white rounded-[40px] border border-slate-100 shadow-sm hover:shadow-xl hover:shadow-slate-200/50 transition-all p-8 flex flex-col relative group overflow-hidden">
                 ...
                 <div className="mt-8 pt-6 border-t border-slate-50 flex justify-between items-center relative z-10">
                   <div className="flex items-center gap-2 text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                     ...
                     Ult. Ejecucion: {new Date(rule.updated_at || rule.created_at).toLocaleString()}
                   </div>
         ...
         <div className="bg-white rounded-[40px] border border-slate-100 shadow-xl p-8">
           {loadingJobs ? (
             <div className="py-12 text-center text-xs font-bold text-slate-400">Cargando ejecuciones...</div>
           ) : jobs.length === 0 ? (
             <div className="text-center py-12">
               ...
               <p className="text-sm text-slate-500 max-w-sm mx-auto mt-2 font-medium">Monitorea cada mensaje enviado por el motor de automatizacion en tiempo real.</p>
             </div>
           ) : (
               ...
               <table className="w-full text-left">
                 <thead className="bg-slate-50/50 border-b border-slate-100">
                   <tr>
                     ...
                     <th className="px-6 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest">Regla</th>
                 ...
                 <tbody className="divide-y divide-slate-50">
                   {jobs.map((job) => (
                     <tr key={job.id} className="hover:bg-slate-50">
                       <td className="px-6 py-4 text-xs font-bold text-slate-700">#{job.id}</td>
                       <td className="px-6 py-4 text-xs font-medium text-slate-600">{job.rule_id}</td>
                       <td className="px-6 py-4 text-xs font-bold text-slate-600">{job.status}</td>
                     </tr>
                   ))}
                 </tbody>
               </table>
