...
const ChatTemplatesAdmin: React.FC = () => {
  ...
  const handleSave = async () => {
    ...
    try {
      const payload: Partial<MessageTemplate> = {
        ...editingTemplate,
        variables_json: extractVariables(editingTemplate.body_text),
        scope: editingTemplate.scope || 'global',
      };
      const saved = await ChatService.saveTemplate(payload);
      if (!saved) throw new Error('No se pudo guardar la plantilla');
  ...
  return (
    <div className="p-4 md:p-8 max-w-[1600px] mx-auto space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      {loading ? (
        <div className="py-10 text-center text-xs font-bold text-slate-400">Cargando plantillas...</div>
      ) : (
                ...
                <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                   ...
                   <button
                    onClick={async () => {
                      ...
                      try {
                        await ChatService.deleteTemplate(template.id);
