...
### Ultimo avance
- Ajuste posterior importante de la capa de pantallazos: al redeployar se detecto que `studiocore-api-{pruebas,produccion}` reiniciaba porque `ScrapingService` intentaba construir el cliente Supabase en el constructor aun cuando `SUPABASE_URL`/`SUPABASE_SERVICE_ROLE_KEY` no estaban presentes en las variables del contenedor. Se corrigio para que el cliente sea opcional y solo se use si la configuracion existe; ademas `deploy/vps/docker-compose.studiocore.{staging,production}.yml` ahora inyecta `SUPABASE_URL` y `SUPABASE_SERVICE_ROLE_KEY` dedicados para el modulo de scraping/debug. Con eso el API vuelve a levantar sano aunque el debug no este configurado, y cuando las variables estan presentes ya puede firmar capturas desde Storage sin tumbar todo el servicio.
- Se implemento la primera capa reusable de depuracion visual para scraping en `Streamate`. La logica elegida para escalar a otras plataformas fue: registrar cada etapa del worker (`LOGIN`, `NAVIGATION`, `EXTRACTION`, `PARSING`) en `scraping_attempts`, capturar un pantallazo al cerrar cada etapa y tambien otro en fallo (`<stage>-error`), subirlo a un bucket privado `scraping-debug` en Supabase Storage, guardar la ruta en `scraping_attempts.screenshot_path`, y exponer las capturas desde API con URLs firmadas (`GET /scraping/jobs/:jobId/screenshots`). En frontend `apps/dashboard/components/ScrapingPaginasPage.tsx` ahora muestra galeria + preview por intento, y `apps/dashboard/ScrapingService.ts` consume ese endpoint firmado. Para que funcione en deploy se endurecio el stack: `studiocore-erp/apps/api/Dockerfile` instala Chromium, `deploy/vps/docker-compose.studiocore.{staging,production}.yml` monta `../../scripts/scrapers` dentro del contenedor API, y `studiocore-erp/apps/api/package.json` incorpora `playwright` + `dotenv`. Validaciones de esta fase: dashboard `tsc` + `build` OK, API `tsc` + `npm run test --workspace @studiocore/api` OK. Pendiente menor al cerrar esta sesion: quedaron sin versionar tres scripts diagnosticos locales (`studiocore-erp/test-ping.mjs`, `studiocore-erp/test-run.mjs`, `studiocore-erp/test-supabase.mjs`) que no hacen parte del runtime principal.
- Se avanzo fuerte sobre los faltantes detectados en los videos de `problemas/Problema` dentro del dashboard legacy y el backend de scraping. Ya quedaron implementados y publicados los modulos/rutas `supervision_tareas`, `gestion_pseudonimos`, `terceros`, `metas_bonos`, `panel_vps` (alias del bloque de scraping) y `nomina_sugerida` (alias de liquidacion/model payroll). En frontend se agregaron `apps/dashboard/components/TaskSupervisionPage.tsx`, `apps/dashboard/components/PseudonymManagementPage.tsx` y `apps/dashboard/components/BonusGoalsPage.tsx`; `TaskSupervisionPage` consume `dashboard/tasks` y persiste revision manual por estudio en `settings`, `PseudonymManagementPage` opera `models_accounts` del estudio activo como gestion de pseudonimos/plataformas, y `BonusGoalsPage` mezcla CRUD sobre `models_goals` con un blueprint creativo persistido en `settings`. Tambien se agrego `terceros` al `resourceConfig`, nuevas entradas en sidebar/routing/admin guards y `ScrapingService.ts` ahora apunta por defecto al backend nuevo via `/erp-api/api/v1` en vez del `/api` legacy.
- En scraping backend tambien quedaron capturados cambios locales utiles no publicados: `studiocore-erp/apps/api/src/modules/scraping/scraping.service.ts` ahora crea primero el `scraping_job` via Supabase Admin antes de lanzar el worker y le pasa `job_id` al proceso hijo; `scraping.controller.ts` ya devuelve ese `jobId`, `scripts/scrapers/streamate/streamate-worker.mjs` soporta reutilizarlo y `studiocore-erp/apps/api/package.json` / `package-lock.json` incorporan `@supabase/supabase-js`. Validacion de esta iteracion: `tsc` del dashboard OK (heap ampliado), `node scripts/build-dashboard.mjs` OK, `tsc` de `studiocore-erp/apps/api` OK y `npm run test --workspace @studiocore/api` OK.
- Aun quedan detalles finos de parity visual respecto a los videos (por ejemplo mas telemetria tipo PM2/servicios en el panel VPS, mas capas de workflow para revision humana y formularios mas ricos alrededor de terceros/metas), pero ya existe la estructura real y operativa para continuar sobre los modulos correctos en vez de partir de cero.
- Se dejo tambien registrada la metodologia reusable para scraping multi-plataforma a partir de videos/procesos reales: (1) extraer storyboards/contact sheets de cada video con `ffmpeg` para identificar vistas, tablas, modales y flujo; (2) agrupar por modulo funcional (`panel_vps`, `workers`, `cola`, `logs`, `errores`, `credenciales por sitio`, `revision manual`, etc.); (3) implementar el worker por sitio en `scripts/scrapers/<sitio>/`, con rate limiter, clasificador de errores y migraciones SQL propias; (4) exponer el disparo/control desde API (`studiocore-erp/apps/api/src/modules/scraping/*`) sin persistir credenciales en claro; y (5) centralizar la operacion/consulta en dashboard con un modulo unico (`scraping_paginas`) que luego se puede extender a Streamate, Chaturbate, Bongacams, LiveJasmin u otras plataformas manteniendo la misma estructura de jobs, attempts, resultados y control manual. Esta metodologia debe reutilizarse para las proximas plataformas que el usuario envie.
- Se detectaron y validaron cambios locales no publicados relacionados con scraping multi-plataforma: integracion del modulo `scraping_paginas` en el dashboard legacy (`apps/dashboard/appRoutes.tsx`, `apps/dashboard/constants.ts`), nuevo `ScrapingService.ts`, nueva UI `apps/dashboard/components/ScrapingPaginasPage.tsx`, y un modulo backend inicial en `studiocore-erp/apps/api/src/modules/scraping/*` que expone `POST /scraping/streamate/run` y dispara un worker Playwright ubicado en `scripts/scrapers/streamate/streamate-worker.mjs`. Para no perder ese trabajo se validaron con `tsc`/build/test (usando heap ampliado donde hizo falta) y se publicaron tambien en GitHub.
- Verificacion final posterior al hardening: durante la primera comprobacion publica despues del commit `6a66ba4` el endpoint `https://terminado.livstre.com/erp-api/api/v1/health` devolvio `502` porque en ese momento no estaban arriba los contenedores `studiocore-*-produccion`. Se reprodujo exactamente el comando SSH del workflow `Deploy Production VPS`, se confirmo que ya completa OK con el `publish.sh` corregido y se dejo de nuevo produccion levantada. Estado publico final tras esa reejecucion: `terminado` y `pruebas` sirven el mismo bundle legacy (`/assets/index-DNxto38u.js`), `https://terminado.livstre.com/erp/` vuelve a servir StudioCore ERP, y ambos `/erp-api/api/v1/health` responden `200`. El ultimo run historico fallido de `Deploy Production VPS` queda como antecedente, pero el flujo actual ya esta sano y comprobado extremo a extremo.
- Se hizo una pasada final de verificacion/correccion global. Hallazgos y cierre: (1) el espejo `pruebas` -> `terminado` ya quedo activo tanto para legacy raiz como para `StudioCore ERP`; ambos dominios publicos hoy sirven el mismo bundle legacy (`/assets/index-DNxto38u.js`) y ambos ERP responden `200` en `/erp-api/api/v1/health`; (2) el deploy automatico de produccion por `push` a `main` seguia siendo fragil por archivos `.secure/*.env.local` con CRLF subidos desde Windows, lo que hacia que `publish.sh` omitiera variables `STUDIOCORE_PRODUCTION_*` al hacer `source`; se corrigio `deploy/vps/publish.sh` para normalizar `\r` antes de cargar envs; (3) tambien se publicaron los cambios locales restantes de monetizacion en `apps/dashboard/MonetizationService.ts` y `apps/dashboard/components/MonetizationPage.tsx` (commit `99fff6c`: borrado de terceros/liquidaciones, retencion personalizada mas robusta y flujo de preview/comision enriquecido). Luego se redeployo manualmente `staging` y `production`, quedando ambos entornos alineados y con contenedores `studiocore-{api,web}-{pruebas,produccion}` arriba en VPS.
- Estado GitHub al cierre: `supabase-migration-final-safe`, `staging` y `main` quedaron en `99fff6c` despues de publicar `fix(deploy): normalize vps env files before sourcing` y `feat(monetization): add deletion flow and richer retention editing`. `Validate Application` y `Secret Scan` del push mas reciente ya estan en verde para `main` y `staging`; `Deploy Production VPS` del nuevo flujo por push a `main` estaba corriendo al cierre de esta sesion, pero el mismo contenido ya fue desplegado manualmente y validado en vivo.
- Se dejo automatizado el siguiente paso que el usuario esperaba: `Deploy Production VPS` ya no queda solo manual, ahora tambien dispara con `push` a `main` en `.github/workflows/production-deploy.yml`. Con esto, si seguimos empujando cambios a `main` (y manteniendo el modo espejo actual), `terminado` deberia redeployarse automaticamente sin tener que entrar otra vez al VPS a lanzarlo a mano.
- Se completo tambien el modo espejo para el flujo legacy del dashboard raiz, no solo para StudioCore ERP. Se actualizaron los valores `TERMINADO_VITE_SUPABASE_URL`, `TERMINADO_VITE_SUPABASE_ANON_KEY`, `TERMINADO_VITE_API_URL`, `TERMINADO_API_URL` y `TERMINADO_NGINX_API_UPSTREAM` en `.secure/vps.terminado.env.local` para que reutilicen la misma configuracion efectiva de `pruebas`, y luego se redeployo produccion otra vez desde worktree limpio. Verificacion real posterior: `https://terminado.livstre.com/` y `https://pruebas.livstre.com/` ya sirven el mismo bundle legacy (`/assets/index-D97rqXsw.js`) y tiempos muy parecidos; en paralelo `https://terminado.livstre.com/erp/` y `https://terminado.livstre.com/erp-api/api/v1/health` siguen funcionando en modo espejo con StudioCore. En resumen, tanto el dashboard legacy como el ERP nuevo en `terminado` ahora reflejan el mismo backend/datos/configuracion que `pruebas`.
- Ademas se detectaron tres archivos versionados con cambios locales pendientes (`apps/dashboard/MonetizationService.ts`, `apps/dashboard/components/MonetizationPage.tsx`, `apps/dashboard/types.ts`) y, para cumplir la peticion del usuario de mantener GitHub igual a esta maquina, se publicaron en el commit `ece26b2` (`feat(monetization): improve beneficiary editing and invoice preview`) sobre `supabase-migration-final-safe`, `staging` y `main`. Se redeployo staging y produccion despues del push para que esos cambios queden vivos en ambos dominios espejo.
- El usuario confirmo que queria comportamiento espejo entre `pruebas` y `terminado` tambien para cambios de datos del ERP (por ejemplo claves/usuarios ajustados en pruebas). Se aplico ese modo espejo para StudioCore: se agregaron en `.secure/vps.terminado.env.local` las variables `STUDIOCORE_PRODUCTION_*` reutilizando el mismo `DATABASE_URL`, `DATABASE_SCHEMA=studiocore_erp`, `JWT_*`, `REDIS_URL` y rutas base de staging, y luego se redeployo produccion desde worktree limpio con `publish.sh production`. Resultado real: ahora `https://terminado.livstre.com/erp/` sirve StudioCore (ya no el dashboard legacy), `https://terminado.livstre.com/erp-api/api/v1/health` responde `{"status":"ok"...}` y el login remoto de `admin@studiocore.local` en terminado devuelve 201. En otras palabras, tanto `pruebas` como `terminado` ya apuntan al mismo backend/datos del ERP, asi que los cambios hechos dentro del ERP en pruebas tambien se reflejan en terminado.
- Se verifico especificamente la brecha entre `pruebas` y `terminado`. Hallazgo: `main` si estaba actualizado en GitHub, pero `terminado` no se habia redeployado con esos cambios porque `Deploy Production VPS` es manual (`workflow_dispatch`) y no se ejecuta por cada push como staging. Se hizo deploy manual de `main` a produccion desde VPS usando worktree limpio y `publish.sh production`, con lo que el dashboard legacy de `https://terminado.livstre.com/` ya quedo recompilado con el commit vigente `d65fcf5` y tiempos publicos alrededor de `0.46s` TTFB. Sin embargo, `https://terminado.livstre.com/erp/` sigue entregando el dashboard legacy (no StudioCore) porque el entorno remoto `.secure/vps.terminado.env.local` no tiene ninguna variable `STUDIOCORE_PRODUCTION_*` ni `TERMINADO_NGINX_STUDIOCORE_*`; por tanto, no existen contenedores `studiocore-*-produccion` ni proxy `/erp` real en terminado. Concluson: el codigo ya esta en produccion para el sistema legacy, pero el ERP nuevo no puede quedar habilitado en `terminado` hasta definir su `DATABASE_URL`/`JWT_*`/routing de produccion.
