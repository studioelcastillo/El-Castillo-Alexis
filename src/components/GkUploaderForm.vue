  ...
  <div>
    <div class="row q-col-gutter-sm">
            ...
            <div class="row no-wrap items-center q-pa-sm q-gutter-xs">
              ...
              <div class="col">
                <div class="q-uploader__title">
                  {{
                    multiple
                      ? "Peso máximo total entre todos los archivos:"
                      : "Peso máximo:"
                  }}
                  {{ parseInt(maxTotalFilesize / 1024 / 1024) }}M
                </div>
      ...
      <div v-if="filesUploaded.length > 0" class="col-xs-12 col-sm-12">
        <h6 class="is-size-3">
          {{ multiple ? "Archivos Cargados" : "Archivo Cargado" }}
        </h6>
        <br />
        <q-card
        ...
        >
          ...
          >
            <div
              class="col text-caption text-grey q-pr-sm"
            ...
            <div class="col-auto">
              <div class="col-auto text-subtitle1">
                <q-avatar
                  rounded
                  :color="getFileData(file.filename).color"
                  text-color="white"
                  :icon="getFileData(file.filename).icon"
                />
              </div>
              <br />
              <q-space />
...
export default defineComponent({
  ...
  methods: {
    async getMaxSize() {
      ...
      this.maxTotalFilesize = 50 * 1024 * 1024; // 50MB default
    },
    getFileData(filename) {
      let ext = filename.split(".").pop();
      var response = this.extensions["DEFAULT"];
      if (
        Object.prototype.hasOwnProperty.call(this.extensions, ext.toUpperCase())
      ) {
    ...
    async uploadFiles() {
      ...
      const files = uploader.queuedFiles;

      for (const file of files) {
        file.__status = "uploading";
        try {
          const path = `${this.uploadUrl}/${Date.now()}_${file.name}`;
          ...
          this.onFileUploadedFailed({
            files: [file],
            xhr: { response: JSON.stringify({ error: error.message }) },
          });
        }
      }
    },
    // Keep compatibility for now but it's not used as we'll use uploadFiles
    uploadFile(files) {
