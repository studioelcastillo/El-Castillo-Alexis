...
const parseArrayParam = (params, key) => {
...
const buildOwnerMap = async (ownerIds) => {
...
const addSummaryIncome = (summaryMap, sourceKey, appKey, totals) => {
...
const finalizeSummary = (summaryMap) =>
  Array.from(summaryMap.values()).map((source) => ({
    ...source,
    incomes: Array.from(source.incomes.values()),
  }));

const aggregateStreamsByModel = async (streams) => {
  const ownerIds = [...new Set((streams || []).map((row) => row.studios?.user_id_owner).filter(Boolean))];
  ...
  const dataset = Array.from(groups.values()).map((group) => {
    const incomes = Array.from(group.incomes.values()).map((income) => ({
      ...income,
    ...
    return {
      ...
      modacc_username: group.modacc_username.size ? Array.from(group.modacc_username).join(";") : null,
...
const aggregateStreamsByStudio = async (streams) => {
  const ownerIds = [...new Set((streams || []).map((row) => row.studios?.user_id_owner).filter(Boolean))];
  const ownerMap = await buildOwnerMap(ownerIds);
...
export default {
  ...
  async getModelsLiquidation(params) {
    ...
    const stdIds = parseArrayParam(queryParams, "std_ids[]").map(Number).filter((v) => !Number.isNaN(v));
    ...
    let filtered = data || [];
    if (apps.length) {
      filtered = filtered.filter((row) => apps.includes(row.models_accounts?.modacc_app));
    }
    if (shiftIds.length) {
    ...
    if (stdActive === "true" || stdActive === "false") {
      ...
      filtered = filtered.filter((row) => row.studios?.std_active === boolActive);
    }
  ...
  async getStudiosLiquidation(params) {
    const queryParams = normalizeQueryString(params.query);
    const since = queryParams.get("report_since");
    const until = queryParams.get("report_until");
    const stdIds = parseArrayParam(queryParams, "std_ids[]").map(Number).filter((v) => !Number.isNaN(v));
    ...
    if (stdAllyMasterPays === "true" || stdAllyMasterPays === "false") {
      ...
      filtered = filtered.filter(
        (row) => row.studios?.std_ally_master_pays === boolPays
      );
    }
