...
const formatRelativeMinutes = (dateValue) => {
...
const resolveRoomTypeId = async (typeName, stdId) => {
...
const mapWarehouseMovement = (row, roomCodeMap) => ({
...
export default {
  async getRooms(filters) {
    ...
    const roomIds = (roomsData || []).map((row) => row.stdroom_id).filter(Boolean);
    ...
    const userIds = (assignmentRows || [])
      .flatMap((row) => [row.user_id_model, row.user_id_monitor])
      .filter(Boolean);
  ...
  async getRoomDetails(params) {
    const roomId = params?.id || params;
    const { data, error } = await supabase
      .from("studios_rooms")
      .select("*")
      .eq("stdroom_id", roomId)
      .single();
  ...
  async createRoom(params) {
    ...
    return { data: { data: room, status: "Success" }, error: null };
  },

  async updateRoom(params) {
    const roomId = params?.id || params?.roomId || params?.stdroom_id || params?.room_id;
    const updates = params || {};
    const stdId = DEFAULT_STUDIO_ID;
    const roomTypeId = updates.type ? await resolveRoomTypeId(updates.type, stdId) : null;

    if (updates.inventory) {
      await supabase.from("room_inventory").delete().eq("stdroom_id", roomId);
  ...
  async removeItemFromRoom(params) {
    const itemId = params?.id || params?.itemId || params;
    const { error } = await supabase.from("room_inventory").delete().eq("room_inventory_id", itemId);
  ...
  async getWarehouseItems() {
    ...
    return { data: { data: (data || []).map(mapWarehouseItem) }, error };
  },

  async createWarehouseItem(params) {
    const item = params || {};
    const payload = {
    ...
    return { data: { data: data ? mapWarehouseItem(data) : null, status: error ? "Error" : "Success" }, error };
  },

  async updateWarehouseItem(params) {
    const id = params?.id || params?.itemId || params?.warehouse_item_id;
    const data = params || {};
    const payload = {
      item_name: data.name,
    ...
    return { data: { data: row ? mapWarehouseItem(row) : null, status: error ? "Error" : "Success" }, error };
  },

  async adjustStock(params) {
    const itemId = params?.itemId || params?.id || params?.warehouse_item_id;
    const movement = params?.movement || params;
    ...
    if (!itemRow) {
      return { data: { status: "Error", message: "Item no encontrado" }, error: new Error("Item no encontrado") };
    }
    ...
    if (["ASSIGN", "LOST", "DAMAGED", "RETURN"].includes(movement.type)) {
      if (movement.type === "RETURN") {
        ...
        if (newStock < qty) {
          return { data: { status: "Error", message: `Stock insuficiente (${newStock}) para realizar esta operacion.` }, error: new Error("Stock insuficiente") };
        }
        newStock -= qty;
      }
    } else if (movement.type === "ADJUSTMENT") {
      if (newStock + movement.qty < 0) {
        return { data: { status: "Error", message: "El ajuste resultaria en stock negativo." }, error: new Error("Stock insuficiente") };
      }
  ...
  async getItemMovements(params) {
    ...
    const roomIds = (data || []).map((row) => row.related_stdroom_id).filter(Boolean);
    ...
    const roomCodeMap = new Map(
      (rooms || []).map((row) => [row.stdroom_id, row.stdroom_code || row.stdroom_name])
    );
    ...
    return { data: { data: movements }, error };
  },

  async assignToRoom(params) {
    const roomId = params?.roomId || params?.stdroom_id || params?.room_id;
    const itemId = params?.itemId || params?.warehouse_item_id;
    ...
    if (currentStock < qty) {
      return { data: { status: "Error", message: "Stock insuficiente en almacen" }, error: new Error("Stock insuficiente") };
    }
  ...
  async createAssignment(params) {
    ...
    const payload = {
      std_id: DEFAULT_STUDIO_ID,
      stdroom_id: data.room_id,
      user_id_model: data.model_id,
      user_id_monitor: data.monitor_id,
    ...
    return { data: { data: assignment, status: error ? "Error" : "Success" }, error };
  },
  ...
  async createTicket(params) {
    ...
    if (ticketId) {
      const { data: updated } = await supabase
        .from("room_tickets")
        .update({
          room_assign_id: data.assignment_id,
          stdroom_id: data.room_id,
    ...
    if (!row) {
      return { data: { status: "Error", message: "No se pudo guardar el ticket" }, error: new Error("No se pudo guardar el ticket") };
    }
    ...
    return { data: { data: ticket, status: "Success" }, error: null };
  },

  async getTicketsByAssignment(params) {
    const assignmentId = params?.id || params?.assignmentId || params;
    const { data: ticketRows, error } = await supabase
      .from("room_tickets")
      .select("*")
      .eq("room_assign_id", assignmentId)
      .order("created_at", { ascending: false });

    const ticketIds = (ticketRows || []).map((row) => row.room_ticket_id);
    const { data: itemRows } = ticketIds.length
      ? await supabase
          .from("room_ticket_items")
          .select("*")
          .in("room_ticket_id", ticketIds)
      : { data: [] };
  ...
  async getRoomHistory(params) {
    const roomId = params?.id || params?.roomId || params;
    const history = [];
  ...
  async getAlerts() {
    ...
    return { data: { data: alerts }, error };
  },

  async getSystemAlerts(params) {
    const status = params?.status || params || "OPEN";
  ...
  async resolveSystemAlert(params) {
    const alertId = params?.id || params?.alertId || params;
  ...
  async getOccupancyStats(params) {
    const date = params?.date || params?.assign_date || "";
    const shift = params?.shift || "";
    const roomsResult = await this.getRooms({
      date,
      shift,
    ...
    const percentage = total_active - maintenance > 0
      ...
      : 0;
    const stats = { total_active, occupied, available, maintenance, percentage };
    return { data: { data: stats }, error: null };
  },

  async getOperationalRankings(params) {
    ...
    const { data: assignments } = await supabase
      ...
      .in(
        "room_assign_id",
        ticketRows.map((row) => row.room_assign_id).filter(Boolean)
      );
    ...
    (ticketRows || []).forEach((ticket) => {
      const assign = (assignments || []).find((a) => a.room_assign_id === ticket.room_assign_id);
      if (!assign) return;
      const userId = assign[roleKey];
      if (!userId) return;
    ...
    const rankings = Array.from(userMap.values()).sort((a, b) => b.score - a.score);
    return { data: { data: rankings }, error: null };
  },
  ...
  async getRoomTypes() {
    ...
    return { data: { data: roomTypes }, error };
  },
  ...
  async updateRoomType(params) {
    const id = params?.id || params?.room_type_id;
    const data = params || {};
    const { data: row, error } = await supabase
      .from("room_types")
      .update({
  ...
  async deleteRoomType(params) {
    const id = params?.id || params?.room_type_id || params;
    const { error } = await supabase.from("room_types").delete().eq("room_type_id", id);
    return { data: { status: error ? "Error" : "Success" }, error };
  },
};
