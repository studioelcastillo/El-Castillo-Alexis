...
const crudFor = (table, select = '*') => ({
  async getAll (query = {}) {
    ...
    Object.entries(query).forEach(([k, v]) => { if (v !== undefined && v !== null) req = req.eq(k, v) })
    return req
  },
...
export const StudioService = {
  ...crudFor('studios', '*, locations(*), users!owner_user_id(user_name, user_surname)'),
  async getAll (query = {}) {
    let req = supabase.from('studios').select('*, locations(*), users!owner_user_id(user_name, user_surname)')
    if (query.std_id) req = req.eq('std_id', query.std_id)
    if (query.std_active !== undefined) req = req.eq('std_active', query.std_active)
    return req
  },
  async uploadImage (std_id, file) {
    const path = `studios/${std_id}/${file.name}`
    const { data, error } = await supabase.storage.from('el-castillo').upload(path, file, { upsert: true })
    if (error) return { data: null, error }
    const { data: urlData } = supabase.storage.from('el-castillo').getPublicUrl(path)
    return supabase.from('studios').update({ std_photo_url: urlData.publicUrl }).eq('std_id', std_id)
  },
  async getSedes (parent_std_id) {
    return supabase.from('studios').select('*').eq('parent_std_id', parent_std_id)
  }
}
...
export const StudioModelService = {
  ...crudFor('studio_models', '*, users(user_name, user_surname, user_identification, user_email), studios(std_name), studio_rooms(room_name), studio_shifts(shift_name)'),
  async getByStudio (std_id) {
    return supabase.from('studio_models')
      .select('*, users(user_name, user_surname, user_identification), studios(std_name)')
      .eq('std_id', std_id)
...
export const UserService = {
  ...
  async getAll (query = {}) {
    ...
    if (query.user_active !== undefined) req = req.eq('user_active', query.user_active)
  ...
  async uploadImage (user_id, file) {
    ...
    if (error) return { data: null, error }
    const { data: urlData } = supabase.storage.from('el-castillo').getPublicUrl(path)
    return supabase.from('users').update({ user_photo_url: urlData.publicUrl }).eq('user_id', user_id)
  }
}

// ── PAYMENTS ─────────────────────────────────────────────────
export const PaymentService = {
  ...crudFor('payments', '*, users(user_name, user_surname), studios(std_name), periods(per_name)'),
  async getAll (query = {}) {
    let req = supabase.from('payments').select('*, users(user_name, user_surname), studios(std_name), periods(per_name)')
...
export const PaysheetService = {
  ...
  async getAll (query = {}) {
    let req = supabase.from('paysheets').select('*, studios(std_name), periods(per_name)')
    if (query.per_id) req = req.eq('per_id', query.per_id)
    if (query.std_id) req = req.eq('std_id', query.std_id)
    return req
  }
...
export const PeriodService = {
  ...crudFor('periods'),
  async getActive () {
    return supabase.from('periods').select('*').eq('per_active', true).order('per_start_date', { ascending: false })
  }
}
...
export const CommissionService = {
  ...crudFor('commissions', '*, studios(std_name), users(user_name, user_surname), periods(per_name)'),
}
...
export const ExchangeRateService = {
  ...
  async getLatest () {
    return supabase.from('exchange_rates').select('*').order('exrate_date', { ascending: false }).limit(1).single()
  }
}
...
export const TransactionService = {
  ...crudFor('transactions', '*, transaction_types(ttype_name), categories(cat_name), users(user_name, user_surname), studios(std_name), periods(per_name)'),
}
...
export const ModelStreamService = {
  ...crudFor('model_streams', '*, users(user_name, user_surname), studios(std_name)'),
  async getByModel (user_id) {
    return supabase.from('model_streams').select('*, studios(std_name)').eq('user_id', user_id).order('modstr_date', { ascending: false })
  },
  async getAll (query = {}) {
    let req = supabase.from('model_streams').select('*, users(user_name, user_surname), studios(std_name)')
    if (query.user_id) req = req.eq('user_id', query.user_id)
    if (query.std_id) req = req.eq('std_id', query.std_id)
    return req
  }
...
export const ModelStreamFileService = {
  ...
  async getByStream (modstr_id) {
    return supabase.from('model_stream_files').select('*').eq('modstr_id', modstr_id)
  }
}
...
export const ModelStreamCustomerService = {
  ...
  async getByStream (modstr_id) {
    return supabase.from('model_stream_customers').select('*').eq('modstr_id', modstr_id).order('modstrc_tokens', { ascending: false })
  }
}
...
export const ModelGoalService = {
  ...crudFor('model_goals', '*, users(user_name, user_surname), studios(std_name), periods(per_name)'),
  async getByUser (user_id) {
    return supabase.from('model_goals').select('*, periods(per_name), studios(std_name)').eq('user_id', user_id)
  }
}

// ── MODEL TRANSACTIONS ───────────────────────────────────────
export const ModelTransactionService = {
  ...crudFor('model_transactions', '*, users(user_name, user_surname), studios(std_name), periods(per_name)'),
}
...
export const NotificationService = {
  ...
  async markAsRead (notif_id) {
    return supabase.from('notifications').update({ notif_read: true }).eq('notif_id', notif_id)
  }
}
...
export const PetitionService = {
  ...
  async getPending () {
    return supabase.from('petitions').select('*, users!user_id(user_name, user_surname)').eq('pet_status', 'PENDING').order('created_at', { ascending: false })
  }
}
