  ...
  <div class="q-pa-md">
    <div class="row">
      <div class="col-12">
        ...
        <q-card
          v-show="openGate('menu-models_liquidation', sUser.prof_id)"
          flat
          bordered
          class="my-card"
        >
              ...
              <div class="row q-col-gutter-sm">
                ...
                <div class="col-xs-12 col-sm-4">
                  <q-select
                    ...
                    color="teal-10"
                    :bg-color="
                      payroll.payrollPeriod.state === 'ABIERTO'
                        ? 'green-11'
                        : 'orange-11'
                    "
                  >
                ...
                <div class="col-sm-4">
                  <q-select
                    ...
                    :disable="
                      !payroll.studio.value ||
                      !payroll.payrollPeriod.value ||
                      loadingCommissionPeriods ||
                      payroll.payrollPeriod.state === 'LIQUIDADO'
                    "
                    lazy-rules
                    :rules="[
                      (val) =>
              ...
              <div>
                <q-btn
                  ...
                  :disable="
                    ...
                    !payroll.payrollPeriod.value ||
                    (payroll.payrollPeriod.state === 'ABIERTO' &&
                      payroll.commissionPeriods.length === 0)
                  "
                />
                <q-btn
                  v-if="
        ...
        <q-card
          v-show="
            openGate('menu-models_liquidation', sUser.prof_id) &&
            payrollData.length > 0
          "
          flat
          bordered
              ...
              <template v-slot:body="props">
                <q-tr :props="props">
                  ...
                  <q-td key="employee_name" :props="props">
                    <span>
                      ...
                      <a
                        v-if="openGate('show-users', sUser.prof_id)"
                        @click="
                          goTo('users/show/' + props.row.employee_id, '_blank')
                        "
                      ...
                      >
                        <q-icon
                          ...
                          class="text-teal-8"
                        >
                          <q-tooltip :delay="100" :offset="[0, 10]"
                            >Consultar empleado</q-tooltip
                          >
                        </q-icon>
                      </a>
                    </span>
                  </q-td>
                  <q-td key="total_salary" :props="props"
                    >${{ miles(props.row.total_salary || 0) }}</q-td
                  >
                  <q-td key="commissions" :props="props"
                    >${{ miles(props.row.commissions || 0) }}</q-td
                  >
                  <q-td key="total_deducciones" :props="props"
                    >${{ miles(props.row.total_deducciones) }}</q-td
                  >
                  <q-td key="total_neto" :props="props" class="text-bold"
                    >${{ miles(props.row.total_neto) }}</q-td
                  >
                  <q-td key="action" :props="props">
                    <a
                      v-if="props.row.payroll_trans_id"
                      class="text-red"
                      style="cursor: pointer"
                      @click="downloadPayrollPdf(props.row.payroll_trans_id)"
                    >
                      <q-icon size="sm" name="picture_as_pdf" />
                      <q-tooltip :offset="[0, 10]"
                        >Descargar PDF - Certificado Nómina</q-tooltip
                      >
                    </a>
                  </q-td>
                </q-tr>
                ...
                >
                  <q-td colspan="100%">
                    ...
                    >
                      ...
                      <template v-slot:body="subprops">
                        <q-tr :props="subprops">
                          ...
                          <q-td key="porcentaje" :props="subprops"
                            >{{ subprops.row.porcentaje }}%</q-td
                          >
                          <q-td key="valor" :props="subprops"
                            >${{ miles(subprops.row.valor) }}</q-td
                          >
                        </q-tr>
                      </template>
                      ...
                      <template v-slot:bottom-row>
                        <q-tr>
                          ...
                          <q-td class="text-right"
                            ><b
                              >${{
                                miles(getCompositionSalaryTotal(props.row))
                              }}</b
                            ></q-td
                          >
                        </q-tr>
                    ...
                    >
                      ...
                      <template v-slot:bottom-row>
                        <q-tr>
                          ...
                          <q-td key="porcentaje" class="text-center"
                            ><b>20.83%</b></q-td
                          >
                          <q-td key="valor" class="text-right"
                            ><b
                              >${{
                                miles(
                                  getPrestacionesTotal(props.row.prestaciones)
                                )
                              }}</b
                            ></q-td
                          >
                    ...
                    <q-table
                      :rows="
                        getSeguridadSocialEmpleadorRows(
                          props.row.social_security
                        )
                      "
                      :columns="prestacionesColumns"
                      row-key="concepto"
                        ...
                        <q-tr>
                          ...
                          <q-td key="valor" class="text-right"
                            ><b
                              >${{
                                miles(
                                  getSeguridadSocialEmpleadorTotal(
                                    props.row.social_security
                                  )
                                )
                              }}</b
                        ...
                        <q-tr>
                          ...
                          <q-td key="valor" class="text-right"
                            ><b
                              >${{
                                miles(
                                  getTotalParafiscales(props.row.parafiscales)
                                )
                              }}</b
                            ></q-td
                          >
                    ...
                    <q-table
                      v-if="
                        props.row.commission_details &&
                        props.row.commission_details.length > 0
                      "
                    ...
                    >
                      ...
                      <template v-slot:body="subprops">
                        <q-tr :props="subprops">
                          <q-td key="model_name" :props="subprops">{{
                            subprops.row.model_name
                          }}</q-td>
                          <q-td key="commission_percentage" :props="subprops"
                            >{{ subprops.row.commission_percentage }}%</q-td
                          >
                          <q-td key="total_earnings" :props="subprops"
                            >${{ miles(subprops.row.total_earnings) }}</q-td
                          >
                          <q-td key="commission_amount" :props="subprops"
                            >${{ miles(subprops.row.commission_amount) }}</q-td
                          >
                        </q-tr>
                      </template>
                      <!-- footer -->
                      <template v-slot:bottom-row>
                        <q-tr>
                          ...
                          <q-td class="text-right"
                            ><b>${{ miles(props.row.commissions) }}</b></q-td
                          >
                        </q-tr>
                      </template>
                    </q-table>
                    ...
                    >
                      ...
                      <template v-slot:bottom-row>
                        <q-tr>
                          ...
                          <q-td key="valor" class="text-right"
                            ><b
                              >${{ miles(props.row.total_deducciones) }}</b
                            ></q-td
                          >
                        </q-tr>
                      </template>
                ...
                <q-tr
                  v-if="
                    payrollData.length - 1 == payrollData.indexOf(props.row)
                  "
                  style="background-color: black; color: white"
                >
                  <q-td auto-width align="center"></q-td>
                  <q-td key="employee_name"><b>Totales</b></q-td>
                  <q-td key="total_salary" class="text-right"
                    ><b>${{ milesFixed(totals.total_salary) }}</b></q-td
                  >
                  <q-td key="commissions" class="text-right"
                    ><b>${{ milesFixed(totals.total_commissions) }}</b></q-td
                  >
                  <q-td key="total_deducciones" class="text-right"
                    ><b>${{ milesFixed(totals.total_deducciones) }}</b></q-td
                  >
                  <q-td key="total_neto" class="text-bold text-right"
                    ><b>${{ milesFixed(totals.total_neto) }}</b></q-td
                  >
        ...
        <q-dialog v-model="extraHourDialog" persistent>
          <q-card style="min-width: 500px">
            <q-card-section>
              <div class="text-h6">
                Agregar hora extra - {{ selectedEmployee.employee_name }}
              </div>
            </q-card-section>
...
export default {
  ...
  data() {
    return {
      ...
      extraHourTypes: [
        ...
        {
          label: "Nocturna (75%)",
          value: "EXTRA_HOUR_NOCTURNA",
          surcharge: 75,
        },
        {
          label: "Dominical (105%)",
          value: "EXTRA_HOUR_DOMINICAL",
          surcharge: 105,
        },
        {
          label: "Dominical Nocturna (155%)",
          value: "EXTRA_HOUR_DOMINICAL_NOCTURNA",
          surcharge: 155,
        },
  ...
  methods: {
    ...
    async studioChanged() {
      ...
      try {
        ...
        const response = await PayrollService.getPayrollPeriods({
          std_id: this.payroll.studio.value,
        });
        const periodsData = response.data?.data || [];

        if (periodsData.length > 0) {
          for (let period of periodsData) {
            const startDate = (period.payroll_period_start_date || "").split(
              "T"
            )[0];
            const endDate = (period.payroll_period_end_date || "").split(
              "T"
            )[0];
            ...
            this.payrollPeriods.push({
              ...
              smmlv: parseFloat(period.payroll_period_smmlv || 0),
            });
          }
    ...
    async periodChanged() {
      this.submitBtnLabel =
        this.payroll.payrollPeriod.state === "ABIERTO"
          ? "Vista Previa"
          : "Consultar";
      this.payrollData = [];
    ...
    async loadCommissionPeriods() {
      ...
      try {
        ...
        const response = await PayrollService.getCommissionPeriods({
          std_id: this.payroll.studio.value,
        });

        if (response.data?.status === "success") {
          this.availableCommissionPeriods = (response.data.data || []).map(
            (period) => ({
    ...
    async loadUsedCommissionPeriodsFromTransactions() {
      try {
        ...
        const transactionsResponse =
          await PayrollService.getPayrollTransactions(
            this.payroll.payrollPeriod.value
          );
        ...
        if (
          ...
          const commissionPeriodIds = Array.isArray(
            firstTransaction.commission_periods
          )
            ? firstTransaction.commission_periods
            : JSON.parse(firstTransaction.commission_periods || "[]");
        ...
        console.error(
          "Error loading used commission periods from transactions:",
          error
        );
        await this.loadCommissionPeriods();
      } finally {
    ...
    async loadUsedCommissionPeriods(periodIds) {
      ...
      try {
        const response = await PayrollService.getPeriodsByIds(periodIds);
    ...
    async onSubmit() {
      ...
      if (this.payroll.payrollPeriod.state === "ABIERTO") {
        if (
          !this.payroll.commissionPeriods ||
          this.payroll.commissionPeriods.length === 0
        ) {
          this.alert(
            "warning",
            "Debe seleccionar al menos un período de comisión"
          );
          return;
        }
      }
      ...
      try {
        ...
        if (this.payroll.payrollPeriod.state === "LIQUIDADO") {
          ...
          if (transactionsResponse.data?.success) {
            this.payrollData = transactionsResponse.data.data.map(
              (transaction) => ({
                employee_id: transaction.employee_id,
                employee_name: transaction.employee_name || "N/A",
                stdmod_id: transaction.stdmod_id,
                total_salary: parseFloat(transaction.total_salary || 0),
                commissions: parseFloat(transaction.commissions || 0),
                total_deducciones: parseFloat(
                  transaction.total_deducciones || 0
                ),
                total_neto: parseFloat(transaction.total_neto || 0),
                prestaciones: transaction.prestaciones || {},
                social_security: transaction.social_security || {},
                parafiscales: transaction.parafiscales || {},
                salary_composition: transaction.salary_composition || [],
            ...
            this.alert(
              "positive",
              `Transacciones cargadas: ${this.payrollData.length} empleados`
            );
          }
        } else {
          ...
          const response = await PayrollService.previewPayroll({
            ...
            commission_period_ids: this.payroll.commissionPeriods.map(
              (p) => p.value
            ),
          });

          if (response.data?.success) {
            this.payrollData = response.data.data || [];
            this.totals = this.calculateTotals(this.payrollData);

            this.alert(
              "positive",
              `Vista previa generada: ${this.payrollData.length} empleados`
            );
          } else {
            this.alert(
              "negative",
              response.data?.message || "Error generando vista previa"
            );
          }
        }
        ...
        .onOk(async () => {
          try {
            ...
            if (response.data?.success) {
              ...
              this.alert(
                "negative",
                response.data?.message || "Error procesando liquidación"
              );
            }
          } catch (error) {
            this.errorsAlerts(error);
    ...
    downloadPayrollPdf(transactionId) {
      const url = this.getApiUrl(
        `/api/payroll/liquidation/pdf/${transactionId}?access_token=${this.decryptSession(
          "token"
        )}`
      );
    ...
    getPrestacionesTotal(prestaciones) {
      return (
        (prestaciones.cesantias || 0) +
        (prestaciones.prima || 0) +
        (prestaciones.vacaciones || 0)
      );
    },
    ...
    getSeguridadSocialEmpleadorTotal(seguridadSocial) {
      return (
        (seguridadSocial.eps_employer || 0) +
        (seguridadSocial.pension_employer || 0) +
        (seguridadSocial.arl || 0)
      );
    },
    ...
    getTotalParafiscales(parafiscales) {
      return (
        (parafiscales.sena || 0) +
        (parafiscales.icbf || 0) +
        (parafiscales.caja_compensacion || 0)
      );
    },
    ...
    getSalaryCompositionRows(employee) {
      ...
      const rows = [
        {
          ...
          porcentaje: this.calculatePercentage(
      ...
      if (employee.auxilios?.alimentacion > 0) {
        rows.push({
          ...
          porcentaje: this.calculatePercentage(
            employee.auxilios.alimentacion,
            compositionTotal
          ),
      ...
      if (
        employee.extra_hours_detail &&
        employee.extra_hours_detail.length > 0
      ) {
        employee.extra_hours_detail.forEach((extra) => {
          rows.push({
            concepto: `Hora extra ${this.getExtraHourLabel(
              extra.concept_type
            )} (${extra.hours}h × ${extra.surcharge_percentage}%)`,
            porcentaje: this.calculatePercentage(
              extra.total_amount,
              compositionTotal
            ),
    ...
    async saveExtraHour() {
      try {
        ...
        await PayrollService.addConcept({
          payroll_period_id: this.payroll.payrollPeriod.value,
          stdmod_id: this.selectedEmployee.stdmod_id,
          concept_type: this.extraHourForm.type,
          concept_hours: this.extraHourForm.hours,
          concept_hourly_rate: this.extraHourForm.hourly_rate,
          concept_surcharge_percentage: this.extraHourForm.surcharge_percentage,
          concept_total: this.extraHourForm.total,
          concept_description: this.extraHourForm.description,
        });
    ...
    async deleteExtraHour(conceptId) {
      this.$q
        ...
        .onOk(async () => {
          try {
            await PayrollService.deleteConcept(conceptId);
    ...
    getCompositionSalaryTotal(employee) {
      ...
      if (
        ...
        console.log(
          ...
          employee.extra_hours_detail.length
        );
    ...
    calculateTotals(employees) {
      ...
      employees.forEach((emp) => {
        ...
        totals.total_prestaciones += this.getPrestacionesTotal(
          emp.prestaciones
        );
