...
export const decryptSession = (key) => {
  ...
  if (data) {
    var bytes = CryptoJS.AES.decrypt(data, xMisc.methods.passEncrypt())
    if (xMisc.methods.hasJsonStructure(bytes.toString(CryptoJS.enc.Utf8))) {
...
export const xMisc = {
  mixins: [xMiscAlert, xMiscDate, xMiscEditor, xMiscNotification, xMiscCore, xMiscPermission],
  data () {
    return {
    }
  },
  methods: {
    ...
    getFrontUrl (route) {
      ...
      return window.location.href.replace(/:\/\/(.*?)\/.*/, ':\/\/$1') + '/' + route.replace(/^\//, '')
    },
    // get API url route
    getApiUrl (route) {
      // process.env.API_URL.replace: http://gk-threshold-api.bygeckode.com/ >> http://gk-threshold-api.bygeckode.com
      // route.replace: /images/users/223 >> images/users/223
      return process.env.API_URL.replace(/\/$/, '') + '/' + route.replace(/^\//, '')
    },
    ...
    encryptSession (key, data) {
      var d = (this.hasJsonStructure(data)) ? JSON.stringify(data) : data
      localStorage.setItem(key, CryptoJS.AES.encrypt(d, this.passEncrypt()).toString())
    },
    ...
    // encrypt string
    encrypt (secret) {
      var encrypted = CryptoJS.AES.encrypt(secret, this.passEncrypt()).toString()
      return encrypted
    },
    // decrypt string
    decrypt (encrypted) {
      encrypted = encrypted.replace(/ /ig, '+')
      var bytes = CryptoJS.AES.decrypt(encrypted, this.passEncrypt())
      if (this.hasJsonStructure(bytes.toString(CryptoJS.enc.Utf8))) {
        return JSON.parse(bytes.toString(CryptoJS.enc.Utf8))
      } else {
        return bytes.toString(CryptoJS.enc.Utf8)
      }
    ...
    validateDateFormat (val) {
      if (val == '' || !val) { return true }
      const regex = /([1-2][0-9][0-9][0-9]-[0-1][0-9]-[0-3][0-9]|[1-2][0-9][0-9][0-9]\/[0-1][0-9]\/[0-3][0-9])/
      return !!regex.test(val)
    },
    ...
    // ???????
    noNegative (evt) {
      if (evt.key === '-' || evt.key === '+' || evt.key === '.' || evt.key === ',' || evt.key.toUpperCase() === 'E') {
        evt.preventDefault()
      }
    },
    ...
    pascalCase(string) {
      string = string.replace(/_/ig, ' ', string)
      string = string.replace(/(\w)(\w*)/ig, function(g0, g1, g2) { return g1.toUpperCase() + g2.toLowerCase() })
      string = string.replace(/ /ig, '', string)
      return string
    },
    pascalCaseSingular(string) {
      string = string.replace(/(ies_)/ig, 'y_', string)
      string = string.replace(/(ies$)/ig, 'y', string)
      string = string.replace(/(ies)([0-9])/ig, 'y$2', string)
      string = string.replace(/(s_)/ig, '_', string)
      string = string.replace(/(s$)/ig, '', string)
      string = string.replace(/(s)([0-9])/ig, '$2', string)
      string = string.replace(/_/ig, ' ', string)
      string = string.replace(/(\w)(\w*)/ig, function(g0, g1, g2) { return g1.toUpperCase() + g2.toLowerCase() })
      string = string.replace(/ /ig, '', string)
      return string
