                      ...
                      <div key={asset.id} className="aspect-[3/4] bg-white rounded-[32px] overflow-hidden relative group border border-slate-100">
                          ...
                          <div className="absolute inset-0 bg-gradient-to-t from-black/80 to-transparent p-4 flex flex-col justify-end">
                              ...
                              <div className="flex gap-2 mt-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                  <button className="flex-1 py-2 bg-emerald-500 text-white rounded-lg text-[9px] font-black uppercase">Aprobar</button>
                                  <button className="p-2 bg-red-500 text-white rounded-lg"><X size={12}/></button>
                              </div>
                          </div>
                      </div>
                  ))}
            ...
            <div className="relative bg-white w-full max-w-xl rounded-[40px] shadow-2xl p-10">
                ...
                <div className="space-y-6">
                    <div>
                        ...
                        <select className="w-full p-4 bg-slate-50 border border-slate-200 rounded-2xl font-bold outline-none focus:border-amber-500">
                            {MOCK_USERS.filter(u => u.profile?.prof_name === 'MODELO').map(m => <option key={m.user_id}>{m.user_name} {m.user_surname}</option>)}
                        </select>
                    </div>
                    <button onClick={onSubmit} className="w-full py-4 bg-slate-900 text-amber-400 rounded-2xl font-black uppercase tracking-widest hover:bg-black transition-all shadow-xl shadow-slate-900/20">Guardar Tarea</button>
                </div>
            </div>
        </div>
    );
