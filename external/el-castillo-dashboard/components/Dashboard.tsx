...
const Dashboard: React.FC = () => {
    ...
    // Filters State
    const [since, setSince] = useState(PREDEFINED_PERIODS[0].since);
    const [until, setUntil] = useState(PREDEFINED_PERIODS[0].until);
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            const [indRes, taskRes, chartRes] = await Promise.all([
                DashboardService.getIndicators(params),
                DashboardService.getTasks(params),
            ...
            if (!indicators) {
                setError("Error de conexión. Verifica tu acceso a la red.");
            }
        } finally {
            setIsLoading(false);
        }
    }, [userId, since, until, stdId, indicators]);
