...
const AttendancePage: React.FC = () => {
                  ...
                  <tbody className="divide-y divide-slate-50">
                      {[1,2,3,4,5,6].map(i => (
                          <tr key={i} className="hover:bg-slate-50/50 transition-colors">
                              <td className="px-8 py-4 text-xs font-bold text-slate-500">Hoy, 0{8+i}:15 AM</td>
                              <td className="px-8 py-4">
                                  <div className="flex items-center gap-3">
                                      <div className="w-8 h-8 rounded-full bg-slate-100 flex items-center justify-center font-bold text-[10px] text-slate-400">ID: {200+i}</div>
                                      <span className="text-xs font-bold text-slate-800">Usuario Mock {i}</span>
                                  </div>
                              </td>
                              <td className="px-8 py-4 text-center">
                                  <span className={`px-2 py-1 rounded text-[9px] font-black uppercase tracking-widest border ${i % 2 === 0 ? 'bg-amber-50 text-amber-600 border-amber-100' : 'bg-emerald-50 text-emerald-600 border-emerald-100'}`}>
                                      {i % 2 === 0 ? 'OUT' : 'IN'}
                                  </span>
                              </td>
                              <td className="px-8 py-4">
                                  <div className="flex items-center gap-2 text-xs font-medium text-slate-600">
                                      <Database size={12} className="text-slate-400" /> CIK721000{i % 2 === 0 ? '1' : '2'}
                                  </div>
                              </td>
                              <td className="px-8 py-4 text-right">
                                  <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Huella Dactilar</span>
                              </td>
                          </tr>
                      ))}
                  </tbody>
                      ...
                      {MOCK_USERS.map(user => (
                          <tr key={user.user_id} className="hover:bg-slate-50/50 transition-colors">
                              <td className="px-8 py-4 flex items-center gap-3">
                                  <Avatar name={user.user_name} src={user.image} size="xs" />
                                  <span className="text-xs font-bold text-slate-800">{user.user_name} {user.user_surname}</span>
                              </td>
                              <td className="px-8 py-4">
                                  <span className="px-2 py-1 bg-slate-100 text-slate-500 rounded text-[9px] font-black uppercase">{user.profile?.prof_name}</span>
                              </td>
                              <td className="px-8 py-4 text-center">
                                  <input type="text" defaultValue={user.user_id} className="w-20 px-2 py-1 bg-slate-50 border border-slate-200 rounded-lg text-xs font-black text-center outline-none focus:border-amber-500" />
                              </td>
                              <td className="px-8 py-4 text-center">
  ...
  const renderDevices = () => (
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 animate-in fade-in duration-500">
          {[1,2].map(i => (
              <div key={i} className="bg-white p-8 rounded-[40px] border border-slate-100 shadow-sm flex flex-col justify-between hover:shadow-xl transition-all group">
                  <div className="flex justify-between items-start mb-6">
                      <div className={`p-4 rounded-2xl ${i === 1 ? 'bg-emerald-50 text-emerald-600' : 'bg-red-50 text-red-600'}`}>
                          <Monitor size={28} />
                      </div>
                      <span className={`px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-[0.2em] border ${i === 1 ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : 'bg-red-50 text-red-600 border-red-100'}`}>
                          {i === 1 ? 'Online' : 'Offline'}
                      </span>
                  </div>
                  <div>
                      <h4 className="font-black text-slate-900 text-lg tracking-tight">Sede Principal {i === 1 ? 'Entrada' : 'Salida'}</h4>
                      <p className="text-xs font-bold text-slate-400 mt-1 uppercase tracking-widest">SN: CIK721000{i}</p>
                      <div className="mt-4 flex items-center gap-2 text-[10px] font-bold text-slate-500 bg-slate-50 w-fit px-3 py-1 rounded-lg">
                          <Clock size={12} /> Última sincronización: Hace {5 * i} min
                      </div>
                  </div>
          ...
          <div className="bg-slate-900 rounded-[40px] p-10 text-white shadow-2xl relative overflow-hidden">
              ...
              <div className="relative z-10 grid grid-cols-1 md:grid-cols-3 gap-12">
                  <div className="space-y-4">
                      ...
                      <h2 className="text-5xl font-black tracking-tighter tabular-nums">$ 450,200</h2>
                  ...
                  <div className="space-y-4">
                      ...
                      <h2 className="text-5xl font-black tracking-tighter tabular-nums">$ 125,000</h2>
                      ...
                      <tbody className="divide-y divide-slate-50">
                          {data.slice(0,6).map(row => (
                              <tr key={row.id} className="hover:bg-slate-50/50">
                                  ...
                                  <td className="px-8 py-4 text-center font-mono text-xs text-slate-600">$ {(row.debt_minutes * 200).toLocaleString()}</td>
                                  <td className="px-8 py-4 text-center font-mono text-xs text-emerald-600 font-black">{(row.overtime_minutes / 60).toFixed(1)}h</td>
                                  <td className="px-8 py-4 text-right font-black text-slate-900">$ {((row.overtime_minutes * 250) - (row.debt_minutes * 200)).toLocaleString()}</td>
                              </tr>
                          ))}
                      </tbody>
                  </table>
