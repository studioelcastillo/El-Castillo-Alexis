...
const ChatWidget: React.FC = () => {
  ...
  const [unreadCount, setUnreadCount] = useState(2); // Mock initial count
  const [isGlowing, setIsGlowing] = useState(false);

  // Simulate receiving a new message effect
  useEffect(() => {
    const timer = setTimeout(() => {
      setUnreadCount(prev => prev + 1);
      setIsGlowing(true);
      setTimeout(() => setIsGlowing(false), 3000); // Stop glow after 3s
    }, 10000); // Simulate after 10s
