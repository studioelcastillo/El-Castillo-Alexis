...
export type PhotoRequestStatus = 'SENT' | 'PENDING_CONFIRMATION' | 'ACCEPTED' | 'CONFIRMED' | 'PHOTOS_TAKEN' | 'IN_PROGRESS' | 'DELIVERED' | 'RATED' | 'CLOSED' | 'RESCHEDULE_PROPOSED' | 'REJECTED' | 'CANCELLED';
...
export interface PhotoRestrictionConfig {
  restrictionDays: number;
  unlockedUsers: number[];
}

export interface PhotoRestrictionStatus {
...
// --- UTILITY CONTROL (GOOGLE ANTIGRAVITY) ---
...
// Duplicate CostCenter removed, merged above
...
export interface ExpenseCatalog {
  ...
  subcategory?: string;
  type: 'OPERATIVO' | 'FINANCIERO' | 'IMPUESTO';
  recurrent: boolean;
  frequency: 'MENSUAL' | 'BIMESTRAL' | 'TRIMESTRAL' | 'SEMESTRAL' | 'ANUAL' | 'CADA_N_MESES';
  every_n_months?: number;
  fixed_value: boolean;
  base_value?: number;
  currency: string;
  default_scope: 'COMPANY' | 'SEDE' | 'CENTER' | 'AREA';
  scope_id?: string;
  provider_id?: string;
...
export type AssignmentMode = 'PERCENTAGE' | 'PESOS' | 'HYBRID' | 'PRORATA_INCOME';
...
export interface AssignmentRule {
  ...
  mode: AssignmentMode;
  details: {
    lines: { line_id: string; value: number }[]; // value is % or pesos
    fixed_percentage?: number; // for hybrid
  };
  active: boolean;
}

export interface ExpenseLineAssignment {
  id: string;
  expense_record_id: string;
  line_id: string;
  percentage_or_weight: number;
...
export interface ThirdParty {
  ...
  company_id: string;
  name: string;
  type: 'PROVEEDOR' | 'EMPLEADO' | 'SOCIO' | 'OTRO';
  active: boolean;
}
...
export interface FinancialMovement {
  ...
  company_id: string;
  period_id: string;
  type: 'PRESTAMO_OTORGADO' | 'PRINCIPAL_PAGADO' | 'INTERES_COBRADO' | 'OTRO';
  value: number;
  reference_id?: string;
  notes?: string;
}

export interface CompanyFinanceParams {
  id: string;
  company_id: string;
  base_currency: string;
  tax_provision_method: 'PERCENTAGE' | 'TABLE' | 'MANUAL';
  default_tax_rate: number;
  provision_base: 'UG' | 'INCOME';
...
export interface UtilityAuditLog {
  id: string;
  company_id: string;
  user_id: string;
  entity: string;
  entity_id: string;
  action: 'CREATE' | 'UPDATE' | 'DELETE' | 'CLOSE_PERIOD' | 'UPDATE_PAYMENT' | 'UPDATE_SETTINGS';
  before?: any;
  after?: any;
  timestamp: string;
}
...
export interface UtilityKPIs {
  ...
  ug: number; // Utilidad Gerencial
  pi: number; // Provisión Impuestos
  ud: number; // Utilidad Disponible
  margin_ud: number;
  variation_mom?: number;
}
