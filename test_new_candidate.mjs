...
const envName = process.env.TARGET_ENV || 'staging';
const project = loadSupabaseProject(envName);
const PROJECT_ID = project.ref;
const host = envName === 'production' ? 'aws-0-us-east-1.pooler.supabase.com' : 'aws-1-us-east-1.pooler.supabase.com';
const port = 6543;
...
async function test() {
  ...
  try {
    console.log(`Testing password candidate for ${project.label}...`);
    await client.connect();
    console.log(`✅ SUCCESS: The database password is valid.`);
    await client.end();
  } catch (err) {
