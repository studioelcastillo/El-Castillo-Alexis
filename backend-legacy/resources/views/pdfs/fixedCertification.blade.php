...
<head>
  ...
  <title>Certificación Laboral a Término Fijo</title>
  <style>
    * {
...
<body>
  <h1>CERTIFICACIÓN LABORAL A TÉRMINO FIJO</h1>

  <br>

  <p>La suscrita, <b>{{ strtoupper($studio->std_manager_name ?? '[NOMBRE COMPLETO DEL REPRESENTANTE]') }}</b>, identificada con Cédula de Ciudadanía No. <b>{{ strtoupper($studio->std_manager_id ?? '[NÚMERO DE CÉDULA DEL REPRESENTANTE]') }}</b> de Cali, actuando en calidad de Representante Legal de <b>{{$studio->std_company_name ?? '[NOMBRE DE LA EMPRESA]'}}</b>, sociedad debidamente constituida e identificada con NIT <b>{{$studio->nitWithVerificationDigit()}}</b> [Correo Electrónico de la Empresa para Verificación] [Número de Teléfono de la Empresa para Verificación], por medio del presente certifica que:</p>

  <p>El(la) señor(a) <b>{{ $model->fullName() }}</b>, identificado(a) con Cédula de Ciudadanía No. <b>{{$model->user_identification ?? '[NÚMERO DE CÉDULA DEL TRABAJADOR]'}}</b>, expedida en <b>{{ strtoupper($model->user_issued_in ?? '[LUGAR DE EXPEDICIÓN DE LA CÉDULA]') }}</b>, con domicilio en <b>{{ strtoupper($model->city->city_name ?? '[CIUDAD DE DOMICILIO DEL TRABAJADOR]') }}</b>, se encuentra o se encontró vinculado(a) laboralmente con nuestra empresa <b>{{$studio->std_company_name ?? '[NOMBRE DE LA EMPRESA]'}}</b>, bajo las siguientes condiciones:</p>

  <ol>
    <li><b>Tipo de Contrato:</b> Contrato Individual de Trabajo a Término Fijo.</li>
    <li><b>Cargo Desempeñado:</b> {{$studioModel->stdmod_position ?? '[CARGO ESPECÍFICO QUE DESEMPEÑA/DESEMPEÑÓ EL TRABAJADOR]'}}</li>
    <li><b>Fechas de Vinculación:</b></li>
  </ol>

  <ul style="list-style-type: none; padding-left: 40px;">
    <li>&bullet; Fecha de Inicio: <b>{{date('d', strtotime($studioModel->stdmod_start_at)) . ' de ' . strtoupper(monthLetters(date('n', strtotime($studioModel->stdmod_start_at)))) . ' de ' . date('Y', strtotime($studioModel->stdmod_start_at))}}</b></li>
    <li>&bullet; Fecha de Terminación: <b>{{$studioModel->stdmod_finish_at ? date('d', strtotime($studioModel->stdmod_finish_at)) . ' de ' . strtoupper(monthLetters(date('n', strtotime($studioModel->stdmod_finish_at)))) . ' de ' . date('Y', strtotime($studioModel->stdmod_finish_at)) : 'Se encuentra actualmente vigente, Término Fijo.'}}</b></li>
  </ul>

  <ol start="4">
    <li><b>Funciones Principales:</b> Durante el período de vinculación laboral, el(la) señor(a) <b>{{ $model->fullName() }}</b> desempeñó sus funciones contribuyendo activamente al desarrollo y cumplimiento de los objetivos de la empresa. Sus responsabilidades incluyeron, entre otras, el apoyo en la ejecución de procesos operativos y/o administrativos del área de {{$studioModel->stdmod_area ?? '[MENCIONAR EL ÁREA GENERAL]'}}, la gestión y organización de información relevante, la coordinación de actividades específicas de su rol, y la colaboración en la implementación de estrategias internas, siempre bajo los lineamientos y políticas de la empresa.</li>
  </ol>

  <ol start="5">
    <li><b>Salario Devengado:</b> El salario mensual integral más auxilios, devengado por el(la) señor(a) <b>{{ $model->fullName() }}</b> no constitutivos del salario corresponde a la suma de $ {{ number_format($studioModel->stdmod_monthly_salary ?? 0, 0, ',', '.') }} ({{ numberToLetters($studioModel->stdmod_monthly_salary ?? 0) }} PESOS COLOMBIANOS), pagaderos en períodos quincenales, de acuerdo con la política salarial de la empresa.</li>
    <li><b>Desempeño:</b> Durante su período de vinculación, el(la) señor(a) <b>{{ $model->fullName() }}</b> demostró ser una persona responsable, proactiva, comprometida, eficiente, puntual, etc. en el cumplimiento de sus labores, mostrando un alto grado de profesionalismo y compromiso con los objetivos de la empresa.</li>
  </ol>
  ...
  <div class="signature-block">
    ...
    <p>C.C. {{ strtoupper($studio->std_manager_id ?? '[1.144.083.039]') }} de Cali</p>
