...
class LiquidationStudioController extends LiquidationController
{
    private $helper;
    private $log;
    ...
    public function liquidate($report_since, $report_until, $args = null) {
        ...
        $orderBy = !empty($args['orderBy']) ? $args['orderBy'] : 'owner_name, modacc_app' ;
        $having = !empty($args['having']) ? $args['having'] : '1=1' ;
        $include_stream_date = !empty($args['models_stream_date']) ? $args['models_stream_date'] : false;

        // profiles
        $prof_ids_model = implode("','", [Profile::MODELO, Profile::MODELO_SATELITE]);
        $prof_ids_owner = implode("','", [Profile::ESTUDIO]);
        ...
        // where
        $whereTmp = $where;
        $whereTmp.= " AND std_principal = false";
        ...
        $sql = "SELECT
                    ...
                    bka.bankacc_number,
                    bka.bankacc_type,
                    bka.bankacc_beneficiary_name,
                    bka.bankacc_beneficiary_document,
                    bka.bankacc_beneficiary_document_type,
                    us2.user_name || ' ' || us2.user_surname AS owner_name,
                    ma.modacc_app,
                    SUM(liq_price) as sum_price,
                    SUM(liq_earnings_value) as sum_earnings_value,
                    MIN(liq_earnings_trm_studio) as modstr_earnings_trm,
                    ...
                    MAX(liq_earnings_percent_studio) as modstr_earnings_percent_studio,
                    SUM(liq_earnings_cop_studio) as sum_earnings_cop_studio,
                ...
                LEFT JOIN banks_accounts bka ON bka.std_id = sm.std_id AND bka.bankacc_main = true
        ...
        foreach ($records as $r => $row) {
            ...
            $dataset[$row->std_id]['std_id'] = $row->std_id;
            $dataset[$row->std_id]['std_nit'] = $row->std_nit;
            $dataset[$row->std_id]['std_name'] = $row->std_name;
            $dataset[$row->std_id]['std_percent'] = (float) $row->std_percent;
            $dataset[$row->std_id]['std_ally_master_pays'] = $row->std_ally_master_pays;
            $dataset[$row->std_id]['std_rtefte'] = $row->std_rtefte;
            $dataset[$row->std_id]['bankacc_entity'] = $row->bankacc_entity;
            $dataset[$row->std_id]['bankacc_number'] = $row->bankacc_number;
            $dataset[$row->std_id]['bankacc_type'] = $row->bankacc_type;
            $dataset[$row->std_id]['bankacc_beneficiary_name'] = $row->bankacc_beneficiary_name;
            $dataset[$row->std_id]['bankacc_beneficiary_document'] = $row->bankacc_beneficiary_document;
            $dataset[$row->std_id]['bankacc_beneficiary_document_type'] = $row->bankacc_beneficiary_document_type;
            $dataset[$row->std_id]['owner_name'] = $row->owner_name;

            // incomes
            if (!isset($dataset[$row->std_id]['incomes'])) {
                $dataset[$row->std_id]['incomes'] = [];
            }
            if (!isset($dataset[$row->std_id]['incomes'][$row->modacc_app])) {
                $dataset[$row->std_id]['incomes'][$row->modacc_app] = [
            ...
            if ($include_stream_date) {
                if (!isset($dataset[$row->std_id]['commission_sum'])) {
                    $dataset[$row->std_id]['commission_sum'] = [];
                }
                if (!isset($dataset[$row->std_id]['commission_sum'][$row->modstr_date])) {
                    $dataset[$row->std_id]['commission_sum'][$row->modstr_date] = [
                        'sum_price' => 0,
                        'sum_earnings_value' => 0,
                        'modstr_earnings_trm' => 0,
                        'modstr_earnings_percent' => 0,
            ...
            // grand total
            $dataset[$row->std_id]['sum_earnings_usd'] += $row->sum_earnings_usd;
            $dataset[$row->std_id]['sum_earnings_eur'] += $row->sum_earnings_eur;
            $dataset[$row->std_id]['sum_earnings_cop'] += $row->sum_earnings_cop_studio;
            $dataset[$row->std_id]['sum_earnings_tokens'] += $row->sum_earnings_tokens;
            $dataset[$row->std_id]['sum_rtefte_model'] += $row->sum_rtefte_model;
            $dataset[$row->std_id]['sum_rtefte_studio'] += $row->sum_rtefte_studio;

            // toma el mayor tiempo de streaming
            if ($dataset[$row->std_id]['sum_earnings_hours'] < $this->helper->timeToNumber($row->sum_time)) {
                $dataset[$row->std_id]['sum_earnings_time'] = $this->helper->timeToNumber($this->helper->timeToNumber($row->sum_time));
                $dataset[$row->std_id]['sum_earnings_time'] = $this->helper->numberToTime($dataset[$row->std_id]['sum_earnings_time']);
                $dataset[$row->std_id]['sum_earnings_hours'] = $this->helper->timeToNumber($row->sum_time);
            }

            // detail
            $dataset[$row->std_id]['incomes'][$row->modacc_app]['sum_price'] += $row->sum_price;
            $dataset[$row->std_id]['incomes'][$row->modacc_app]['sum_earnings_value'] += $row->sum_earnings_value;
            $dataset[$row->std_id]['incomes'][$row->modacc_app]['modstr_earnings_trm'] = $row->modstr_earnings_trm;
            $dataset[$row->std_id]['incomes'][$row->modacc_app]['modstr_earnings_percent'] = $row->modstr_earnings_percent_studio;
            $dataset[$row->std_id]['incomes'][$row->modacc_app]['sum_earnings_tokens'] += $row->sum_earnings_tokens;
            $dataset[$row->std_id]['incomes'][$row->modacc_app]['sum_earnings_tokens_rate'] = $row->modstr_earnings_tokens_rate;
            $dataset[$row->std_id]['incomes'][$row->modacc_app]['sum_earnings_usd'] += $row->sum_earnings_usd;
            $dataset[$row->std_id]['incomes'][$row->modacc_app]['sum_earnings_eur'] += $row->sum_earnings_eur;
            $dataset[$row->std_id]['incomes'][$row->modacc_app]['sum_earnings_cop'] += $row->sum_earnings_cop_studio;
            $dataset[$row->std_id]['incomes'][$row->modacc_app]['sum_time'] = $this->helper->timeToNumber($dataset[$row->std_id]['incomes'][$row->modacc_app]['sum_time']) + $this->helper->timeToNumber($row->sum_time);
            $dataset[$row->std_id]['incomes'][$row->modacc_app]['sum_time'] = $this->helper->numberToTime($dataset[$row->std_id]['incomes'][$row->modacc_app]['sum_time']);

            if ($include_stream_date) {
                // commission sum
                //$dataset[$row->std_id]['commission_sum'][$row->modstr_date]['sum_price'] += $row->sum_price;
                //$dataset[$row->std_id]['commission_sum'][$row->modstr_date]['sum_earnings_value'] += $row->sum_earnings_value;
                $dataset[$row->std_id]['commission_sum'][$row->modstr_date]['modstr_earnings_trm'] = $row->modstr_earnings_trm;
                //$dataset[$row->std_id]['commission_sum'][$row->modstr_date]['modstr_earnings_percent'] = $row->modstr_earnings_percent_studio;
                //$dataset[$row->std_id]['commission_sum'][$row->modstr_date]['sum_earnings_tokens'] += $row->sum_earnings_tokens;
                //$dataset[$row->std_id]['commission_sum'][$row->modstr_date]['sum_earnings_tokens_rate'] = $row->modstr_earnings_tokens_rate;
                $dataset[$row->std_id]['commission_sum'][$row->modstr_date]['sum_earnings_usd'] += $row->sum_earnings_usd;
                $dataset[$row->std_id]['commission_sum'][$row->modstr_date]['sum_earnings_eur'] += $row->sum_earnings_eur;
                $dataset[$row->std_id]['commission_sum'][$row->modstr_date]['sum_earnings_cop'] += $row->sum_earnings_cop;
                //$dataset[$row->std_id]['commission_sum'][$row->modstr_date]['sum_time'] = $this->helper->timeToNumber($dataset[$row->std_id]['commission_sum'][$row->modstr_date]['sum_time']) + $this->helper->timeToNumber($row->sum_time);
                //$dataset[$row->std_id]['commission_sum'][$row->modstr_date]['sum_time'] = $this->helper->numberToTime($dataset[$row->std_id]['commission_sum'][$row->modstr_date]['sum_time']);
            }
        }
        ...
        $sql = "SELECT DISTINCT
                ...
                FROM users us
                INNER JOIN studios_models sm ON sm.user_id_model = us.user_id
                LEFT JOIN models_accounts ma ON ma.stdmod_id = sm.stdmod_id
                ...
                LEFT JOIN studios_shifts ss ON ss.stdshift_id = sm.stdshift_id
                LEFT JOIN banks_accounts bka ON bka.std_id = sm.std_id AND bka.bankacc_main = true
                WHERE us.prof_id IN ('$prof_ids_owner') AND $whereTmp
                ORDER BY user_name
        ";
        ...
        foreach ($records as $r => $row) {
            ...
            $dataset[$row->std_id]['user_id'] = $row->user_id;
            $dataset[$row->std_id]['user_name'] = $row->user_name;
            $dataset[$row->std_id]['user_surname'] = $row->user_surname;
            $dataset[$row->std_id]['bankacc_entity'] = $row->bankacc_entity;
            $dataset[$row->std_id]['bankacc_number'] = $row->bankacc_number;
            $dataset[$row->std_id]['bankacc_type'] = $row->bankacc_type;
            $dataset[$row->std_id]['bankacc_beneficiary_name'] = $row->bankacc_beneficiary_name;
            ...
            $dataset[$row->std_id]['user_document_type'] = $row->user_document_type;
            $dataset[$row->std_id]['user_beneficiary_name'] = $row->user_beneficiary_name;
            $dataset[$row->std_id]['user_beneficiary_document'] = $row->user_beneficiary_document;
            $dataset[$row->std_id]['user_beneficiary_document_type'] = $row->user_beneficiary_document_type;
            $dataset[$row->std_id]['stdmod_id'] = $row->stdmod_id;
            $dataset[$row->std_id]['std_id'] = $row->std_id;
            $dataset[$row->std_id]['std_name'] = $row->std_name;
            $dataset[$row->std_id]['owner_name'] = $row->owner_name;
            ...
            if ($row->transtype_group == 'EGRESOS') {
                // discounts
                if (!isset($dataset[$row->std_id]['discounts'])) {
                    $dataset[$row->std_id]['discounts'] = [];
                }
                ...
                // grand total
                $dataset[$row->std_id]['sum_earnings_discounts'] += $row->trans_total;

                // Separar según tenga o no retención en la fuente
                if ($row->trans_rtefte) {
                    $dataset[$row->std_id]['sum_earnings_base_discount_rtefte'] += $row->trans_total;
                }

                // OTHERS
            } else if ($row->transtype_group == 'INGRESOS') {
                // others
                if (!isset($dataset[$row->std_id]['others'])) {
                    $dataset[$row->std_id]['others'] = [];
                }
                ...
                // grand total
                $dataset[$row->std_id]['sum_earnings_others'] += $row->trans_total;

                // Separar según tenga o no retención en la fuente
                if ($row->trans_rtefte) {
                    $dataset[$row->std_id]['sum_earnings_base_others_rtefte'] += $row->trans_total;
                }
            }
        }
        ...
        $sql = "SELECT DISTINCT
                    ...
                    pmf.payfile_total,
                    pmf.created_by,
                    pm.pay_studios_retefuente_generated,
                    pm.pay_id
                FROM payments pm
                INNER JOIN payments_files pmf ON pmf.payfile_id = pm.payfile_id
                INNER JOIN studios st ON st.std_id = pm.std_id
                WHERE pm.pay_period_since >= '$report_since' AND pm.pay_period_until <= '$report_until'
                ORDER BY payfile_id
        ";
        ...
        foreach ($records as $r => $row) {
            if (isset($dataset[$row->std_id])) {
                $dataset[$row->std_id]['payments'][] = [
                    ...
                    'pay_studios_retefuente_generated' => $row->pay_studios_retefuente_generated,
                    'pay_id' => $row->pay_id
                ];
            }
        }
        ...
        foreach ($dataset as $m => $model) {
            ...
            // Retenciones
            // Validar si el estudio aplica retenciones según std_rtefte
            if (isset($dataset[$m]['std_rtefte']) && $dataset[$m]['std_rtefte'] === true) {
                $rtefte = 4.00; // 4% de retención para estudios
                ...
                // El estudio no aplica retenciones
                $dataset[$m]['sum_earnings_rtefte'] = 0;
            }
    ...
    public function getLiquidation(Request $request) {
        try {
            ...
            if (in_array($uAuth->prof_id, [Profile::JEFE_MONITOR, Profile::MONITOR])) {
                ...
                // $user_ids = $user_controller->getAllDescendantUserIds($uAuth->user_id, true);
                // $where .= " AND us.user_id IN (" . implode(',', $user_ids) . ")";
    ...
    public function getLiquidationPaymentPlain(Request $request) {
        ...
        if (!empty($query['report_until'])) {
            ...
            if (!empty($query['std_ids'])) { // Son los seleccionados con checks
                $where .= " AND st.std_id IN ('" . implode("','", $query['std_ids']) . "')";
            }
            if (in_array($uAuth->prof_id, [Profile::MODELO, Profile::MODELO_SATELITE])) {
                $where .= " AND us.user_id = '" . $uAuth->user_id . "'";
            ...
            if (!empty($query['report_destiny_banks'])) {
                $where .= " AND bka.bankacc_entity IN ('" . implode("','", $query['report_destiny_banks']) . "')";
            }
            ...
            $where.= " AND bka.bankacc_entity IS NOT NULL AND bka.bankacc_number IS NOT NULL AND bka.bankacc_type IS NOT NULL ";
            ...
            foreach ($dataset as $data) {
                ...
                $payment['detail'][] = [
                    'beneficiary_document_type' => $data['bankacc_beneficiary_document_type'],
                    'beneficiary_document' => $data['bankacc_beneficiary_document'],
                    'beneficiary_name' => $data['bankacc_beneficiary_name'],
                    'bank_entity' => $data['bankacc_entity'],
                    'bank_account' => $data['bankacc_number'],
                    'bank_account_type' => $data['bankacc_type'],
                    'sum_earnings_total' => $data['sum_earnings_total'],
                ];
            }

            // file generation
            $std_id = (Profile::ESTUDIO == $uAuth->prof_id && isset($query['std_id'])) ? $query['std_id'] : null;
            if ($std_id != null) {
    ...
    public function getLiquidationBySource($report_since, $report_until, $args = null) {
        ...
        foreach ($dataset as $m => $model) {
            ...
            // netos y totales
            $dataset[$m]['sum_earnings_net'] = $dataset[$m]['sum_earnings_cop'] - $dataset[$m]['sum_earnings_discounts'] + $dataset[$m]['sum_earnings_others'];
            $dataset[$m]['sum_earnings_rtefte'] = $dataset[$m]['sum_rtefte_studio']; // Usa la retención calculada específicamente para el estudio
            $dataset[$m]['sum_earnings_total'] = $dataset[$m]['sum_earnings_net'] - $dataset[$m]['sum_earnings_rtefte'];
