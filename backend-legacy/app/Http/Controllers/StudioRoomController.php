...
class StudioRoomController extends Controller
{
    ...
    public function store(Request $request)
    {
        try {
            ...
            $this->validate($request, [
                ...
                'stdroom_name' => 'required|max:255',
            ]);

            $stdroom_consecutive = StudioRoom::where('std_id', $data['std_id'])->max('stdroom_consecutive');

            // create record
            $record = StudioRoom::create([
                'std_id' => (isset($data['std_id'])) ? $data['std_id'] : null,
                'stdroom_name' => (isset($data['stdroom_name'])) ? $data['stdroom_name'] : null,
                'stdroom_consecutive' => (intval($stdroom_consecutive) + 1),
            ]);
            if ($record) {
                $this->log::storeLog($uAuth, 'studios_rooms', $record->stdroom_id, 'INSERT', null, $record, $request->ip);
                DB::commit();
    ...
    public function show(Request $request)
    {
        try {
            ...
            $record = StudioRoom::with(['studio'])
                ->where($data['where'])
                ->orWhere($data['orWhere'])
                ->whereRaw($data['whereRaw'])
                ->whereNull('deleted_at')
                ->orderBy('stdroom_id', 'desc');
            $record = $this->applyTenantScope($record, $request, 'studios_rooms.std_id')->get();
    ...
    public function update(Request $request, $id)
    {
        try {
            ...
            $record = StudioRoom::findOrFail($id);
            // validamos los datos
            $this->validate($request, [
                'std_id' => 'required|exists:studios,std_id',
                'stdroom_name' => 'required|max:255',
            ...
            $before = StudioRoom::findOrFail($id);
            $record->update([
                'std_id' => (isset($data['std_id'])) ? $data['std_id'] : null,
                'stdroom_name' => (isset($data['stdroom_name'])) ? $data['stdroom_name'] : null,
            ]);
            ...
            if ($record) {
                $this->log::storeLog($uAuth, 'studios_rooms', $record->stdroom_id, 'UPDATE', $before, $record, $request->ip);

                return response()->json(['status' => 'success', 'data' => $record->stdroom_id], 200);
            } else {
        ...
        try {
            ...
            $errors = $this->helper->validateWithMessages($request, [
                'id' => [
                    'required' => '',
                    'integer' => '',
                    'exists:studios_rooms,stdroom_id' => '',
                    'unique:studios_models,stdroom_id' => 'El registro está asociado a "Modelos".',
                ]
            ]);
            if (!empty($errors)) {
                return response()->json([
            ...
            if ($record) {
                $this->log::storeLog($uAuth, 'studios_rooms', $record->stdroom_id, 'DELETE', $before, $record, $request->ip);
                return response()->json(['status' => 'success', 'data' => $record->stdroom_id], 200);
            } else {
                return response()->json(['status' => 'fail'], 500);
            }
    ...
    public function export(Request $request)
    {
        ...
        //////////
        $data = $this->helper::generateConditions($request);
        $records = StudioRoom::with(['studio'])
            ->where($data['where'])
            ->orWhere($data['orWhere'])
            ->whereRaw($data['whereRaw'])
            ->whereNull('deleted_at')
        ...
        $fileName = 'studios_rooms_export.xlsx';
