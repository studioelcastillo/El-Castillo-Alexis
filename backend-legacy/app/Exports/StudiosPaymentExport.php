...
class StudiosPaymentExport
{
    ...
    {
        $studios_payment = $this->getStudiosPayment($term, $action, $consecutive);
        $this->sheet = $this->excel->getSheet(0);
        $this->studiosPaymentSheet($studios_payment);

        
        $fileName = 'Pago-de-estudios-'.date('Y-m-d').'.xlsx';
    ...
    public function getStudiosPayment($term, $action, $consecutive)
    {
    	...
    	$payments = Payment::with(['studio' => function ($query) use ($term) {
    			$query->select('studios.std_id', 'studios.std_name', 'studios.std_nit', 'models_streams.modstr_earnings_trm', 'models_streams.modstr_date')
                ->join('studios_models', 'studios.std_id', 'studios_models.std_id')
                ->join('models_accounts', 'models_accounts.stdmod_id', 'studios_models.stdmod_id')
    			->join('models_streams', 'models_accounts.modacc_id', 'models_streams.modacc_id')
    			->whereBetween('models_streams.modstr_date', $term);
    		}
    	, 'paymentFile', 'studio:std_id,std_name,std_nit'])
    	->where('pay_period_since', $term[0])
    	->where('pay_period_until', $term[1])
        ->whereIn('pay_studios_report_generated', $report_generated)
        ->whereNull('stdmod_id');

        $payments->update(['pay_studios_report_generated' => true]);
    	$payments = $payments->get();

    	$studio_account = '';
    	foreach ($account_accounts as $account) {
			if ($account->accacc_code == 'PALD') {
				$studio_account = $account->accacc_number;
				break;
			}
		}
    	$studios_payment = array();
    	foreach ($payments as $payment) {
    		...
    		$studio_nit = (isset($payment->toArray()['studio']) && isset($payment->toArray()['studio']['std_nit'])) ? $payment->toArray()['studio']['std_nit'] : '';
    		$exchange_rate = (isset($payment->toArray()['studio']['modstr_earnings_trm'])) ? $payment->toArray()['studio']['modstr_earnings_trm'] : '';
    		$row1 = array(
    		...
    		$studios_payment[] = $row1;
    		$studios_payment[] = $row2;
    	}
        $account_voucher->accvou_consecutive = $consecutive + 1;
        $account_voucher->save();
        return $studios_payment;
    }				  		

    //inventory control summary sheets
    public function studiosPaymentSheet($studios_payment) 
    {
        ...
        $this->sheet->fromArray($studios_payment, null, 'A2');
        ...
        // sheet name
        $this->sheet->setTitle(substr('Pago de estudios', 0, 29));
    }
    /**
     */
